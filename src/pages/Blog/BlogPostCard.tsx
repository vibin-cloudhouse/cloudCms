...
import { FormattedBlog } from "./types"; // Import the correct interface
import backgroundImage from "../../../public/lovable-uploads/40454896_8851051.jpg"; // Adjust the path as necessary


interface BlogPostCardProps {
  post: FormattedBlog; // Use the new FormattedBlog interface
}

const BlogPostCard: React.FC<BlogPostCardProps> = ({ post }) => {
  // Function to format date (assuming createdAt is an ISO string)
  ...
  {/* Second image (banner or SVG) above the background */}
  <img
    ...
    <div className="absolute top-3 right-3 bg-teal-500 text-white text-xs font-medium py-1 px-3 rounded-full shadow-sm z-30">
      ...
      <CardHeader className="pb-2">
        <CardTitle className="text-xl font-bold text-slate-800 dark:text-slate-100 group-hover:text-[#006FEE] dark:group-hover:text-teal-400 transition-colors line-clamp-2">
          <Link to={`/blog/${post?.slug}`}>{post?.heading}</Link> {/* Use heading */}
        </CardTitle>
        <CardDescription className="line-clamp-2 mt-2 text-slate-600 dark:text-slate-300">{post?.description}</CardDescription> {/* Use description */}
      </CardHeader>

      <CardContent className="flex-grow">
        <div className="prose-sm text-slate-600 dark:text-slate-300 line-clamp-3">
          {/* Display a snippet of the description or first content block */}
      ...
      <CardFooter className="pt-4 border-t border-slate-100 dark:border-slate-800 flex flex-wrap gap-4 text-xs text-slate-500 dark:text-slate-400">
        <div className="flex items-center gap-1.5">
          ...
          <span>{post?.username || "Unknown Author"}</span> {/* Use username */}
        </div>
        <div className="flex items-center gap-1.5">
